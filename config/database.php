<?php
/**
 * Подключение к БД: MySQL или SQLite (портативный режим)
 */
...
try {
    if (($db['driver'] ?? '') === 'sqlite') {
        $path = $db['path'] ?? __DIR__ . '/../data/europa27.sqlite';
        $dir = dirname($path);
        ...
        $user = trim($db['user'] ?? '');
        $pass = $db['pass'] ?? '';
        $dsn = sprintf('mysql:host=%s;dbname=%s;charset=%s', $host, $name, $db['charset'] ?? 'utf8mb4');
    ...
    $hint = '';
    if (($db['driver'] ?? '') === 'sqlite') {
        $hint = ' Запустите install_portable.php для создания базы.';
    } elseif (strpos($msg, '2002') !== false || strpos($msg, 'No such file') !== false) {
